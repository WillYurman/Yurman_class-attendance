...
- **Permanent per-course QR code** — print once at the start of term; reuse every session. Routes students to whichever session is currently open
...
5. In **Settings → Start Command**, set: `gunicorn run:app`
6. In **Environment**, add: `ANTHROPIC_API_KEY` → your key from console.anthropic.com
7. `SECRET_KEY` is auto-generated by Render — no action needed
